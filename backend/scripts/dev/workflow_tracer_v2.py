"""
Workflow Testing Script for NLP Service V2
Tracks and logs all pathways through the unified LangGraph workflow
"""
...
try:
    ...
    from backend.services.nlp_service_v2 import NLPServiceV2
...
class WorkflowTracerV2:
    """Workflow tracer specifically for NLPServiceV2 with unified workflow"""

    def __init__(self, openai_api_key: str):
        self.nlp_service = NLPServiceV2(openai_api_key)
        self.traced_queries = []
    ...
    async def trace_query(self, query: str, query_id: str = None) -> Dict[str, Any]:
        """
        Trace a single query through the V2 workflow and capture all details
    ...
    def _create_traced_workflow(self, trace: Dict[str, Any]):
        """Create a V2 workflow with tracing capabilities"""
        ...
        # Get the original workflow structure from NLP service V2
        ...
        # Add conditional edges from parse (V2 workflow structure)
        ...
        # Set entry point (V2 starts with "parse")
        workflow.set_entry_point("parse")
        return workflow.compile()

    def _create_traced_node(self, node_name: str, trace: Dict[str, Any]):
        """Create a traced version of a V2 workflow node"""
        ...
        async def traced_node(state: Dict) -> Dict:
            ...
            try:
                ...
                if original_node_func:
                    ...
                    # Fallback to hardcoded node handlers for V2 nodes
                    if node_name == "parse":
                        result = await self._trace_parse_node(state, trace)
                    elif node_name == "read_response":
                        result = await self._trace_read_response_node(state, trace)
                    elif node_name == "write_response":
                        result = await self._trace_write_response_node(state, trace)
                    elif node_name == "unsure_response":
                        result = await self._trace_unsure_response_node(state, trace)
                    else:
                        ...
                        if hasattr(self.nlp_service, method_name):
                            ...
                            raise ValueError(
                                f"Unknown V2 node: {node_name} - no method found"
                            )
    ...
    async def _trace_parse_node(self, state: Dict, trace: Dict) -> Dict:
        """Trace the unified parse node with LLM call logging"""
        try:
            ...
            prompt = self.nlp_service.prompt_manager.generate_unified_prompt(
            ...
            # Log the unified LLM call
            self._log_llm_call("parse", prompt, content, llm_time)
            ...
            try:
                ...
                if operation == "read":
                    ...
                    entries = await self.nlp_service._execute_read_query(query_params)
    ...
    async def _trace_read_response_node(self, state: Dict, trace: Dict) -> Dict:
        """Trace read response node with LLM call logging"""
        try:
    ...
    async def _trace_write_response_node(self, state: Dict, trace: Dict) -> Dict:
        """Trace write response node with LLM call logging"""
        try:
            # Generate response using existing template
            response_prompt = (
    ...
    async def _trace_unsure_response_node(self, state: Dict, trace: Dict) -> Dict:
        """Trace unsure response node with LLM call logging"""
        try:
            # Generate response using existing template
            response_prompt = (
    ...
    async def trace_queries_from_file(self, queries_file: str) -> List[Dict[str, Any]]:
        ...
        for i, query_data in enumerate(queries):
            ...
            expected_v2 = query_data.get("expected_pathway_v2")
            expected = expected_v2 or query_data.get("expected_pathway", "")
            ...
            if expected_v2 and "expected_pathway" in query_data:
                trace["pathway_comparison"]["expected_v1"] = query_data["expected_pathway"]
    ...
    def _has_query_errors(self, query: Dict[str, Any]) -> bool:
        ...
        # Check node-level errors (but exclude "unsure_response" node ambiguous responses)
        ...
        if query.get("nodes"):
            for node_name, node in query["nodes"].items():
                ...
                # Skip "unsure_response" node ambiguous responses
                if node_name == "unsure_response":
                    continue
                ...
                if has_errors:
                    ...
                    if query.get("nodes"):
                        for node_name, node in query["nodes"].items():
                            ...
                            if output_state and output_state.get("error"):
                                node_errors.append(
                                    {
                                        "type": "output_state_error",
                                        "message": str(output_state["error"]),
                                    }
                                )
    ...
    def save_results(self, results: List[Dict[str, Any]], output_file: str = None):
        ...
        if output_file is None:
            ...
            output_file = (
                script_dir / "data" / f"workflow_trace_results_v2_{timestamp}.json"
            )
        ...
        failure_file = str(output_file).replace(
            "workflow_trace_results_v2_", "workflow_failures_v2_"
        )
...
async def main():
    """Main function to run the V2 workflow tracer"""
    ...
    # Initialize tracer
    tracer = WorkflowTracerV2(openai_api_key)
    ...
    for json_file in json_files:
        ...
        try:
            ...
            output_file = (
                script_dir
                / "data"
                / f"workflow_trace_results_v2_{input_stem}_{timestamp}.json"
            )
